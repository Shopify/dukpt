...
module DUKPT
  module Encryption
    ...
    def dek_from_key(key, use_variant)
      key = key.to_i(16)
      key = key ^ DEK_MASK

      if use_variant
        hex_string_from_val(key, 16)
      else
    ...
    def derive_DEK(ipek, ksn, use_variant)
      dek_from_key(derive_key(ipek, ksn), use_variant)
    end
...
if defined?(RUBY_ENGINE) && RUBY_ENGINE == 'jruby'
  require_relative 'jruby/encryption'
end